...
export default function PremixesPage() {
  ...
  async function savePremix() {
    ...
    const ingData = {
      name: premix?.name || 'Premix', category: premix?.category || 'premix',
      species_suitable: premix?.species_suitable || ['cattle','beef','sheep','pig','poultry'],
      nutritionist_id: user.id, premix_id: expandedId, source: 'premix',
      dm_pct: totals.dm > 0 ? totals.dm : 95, cp_pct: totals.cp, me_mj: totals.me,
      ndf_pct: totals.ndf, adf_pct: 0, ee_pct: totals.ee, starch_pct: totals.starch,
      ca_pct: totals.ca, p_pct: totals.p, mg_pct: totals.mg, k_pct: totals.k,
      na_pct: totals.na, s_pct: totals.s, cl_pct: totals.cl, particle_class: 'concentrate',
    }
    const { data: existing } = await supabase.from('ingredients').select('id').eq('premix_id', expandedId).limit(1).single()
    if (existing) { await supabase.from('ingredients').update(ingData).eq('id', existing.id) }
    else { await supabase.from('ingredients').insert(ingData) }
  ...
  return (
    <div className="p-7 max-w-[1200px]">
      <div className="flex items-center justify-between mb-5">
        <div><h1 className="text-2xl font-bold text-text">{t('premixes.title')}</h1><p className="text-sm text-text-ghost mt-0.5">{t('sidebar.premixes')}</p></div>
        <button onClick={() => { setNewName(''); setNewCategory('mineral_mix'); setNewDesc(''); setShowCreate(true) }} className="btn btn-primary"><Plus size={14} /> {t('premixes.new_premix')}</button>
      </div>

      <div className="relative mb-4"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-ghost" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('common.search')} className="input pl-9" /></div>
      ...
      <div className="flex flex-col gap-3">
            ...
            <div key={p.id} className="card">
              ...
              {isExpanded && (
                <div className="border-t border-border px-4 py-3">
                  ...
                  {editIngs.map((ei, idx) => {
                    ...
                    return (
                      <div key={ei.id || idx} className="grid grid-cols-[1fr_80px_60px_50px_45px_20px] px-2 py-1.5 border-b border-border/5 items-center gap-2">
                        <div><div className="text-sm font-semibold text-text-dim truncate">{ing.name}</div><div className="text-[10px] text-text-ghost font-mono">{ing.category} · CP {ing.cp_pct || 0}% · ME {ing.me_mj || 0}</div></div>
                  ...
                  {editIngs.length === 0 && <div className="text-center py-6 text-sm text-text-ghost">{t('premixes.no_premixes')}</div>}
                  ...
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => { setIngSearch(''); setIngCatFilter(''); setShowAddIng(true) }} className="btn btn-ghost btn-sm"><Plus size={14} /> {t('ingredients.add_ingredient')}</button>
                    <div className="flex-1" />
                    <button onClick={savePremix} disabled={saving} className={`btn btn-primary btn-sm ${saved ? 'bg-brand/50' : ''}`}><Save size={14} /> {saved ? `✓ ${t('common.saved')}` : t('common.save')}</button>
                  </div>
                </div>
              )}
            </div>
        ...
        {filtered.length === 0 && <div className="card px-4 py-12 text-center"><Package size={32} className="text-text-ghost mx-auto mb-3" /><p className="text-sm text-text-ghost">{premixes.length === 0 ? t('premixes.no_premixes') : t('common.no_results')}</p></div>}
      </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-surface-card rounded-xl border border-border w-full max-w-md p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-bold text-text">{t('premixes.new_premix')}</h2><button onClick={() => setShowCreate(false)} className="text-text-ghost bg-transparent border-none cursor-pointer"><X size={18} /></button></div>
            <div className="flex flex-col gap-3">
              <div><label className="text-xs font-semibold text-text-muted block mb-1">{t('common.name')} *</label><input value={newName} onChange={e => setNewName(e.target.value)} className="input" placeholder="e.g. Dairy Mineral Mix" /></div>
              ...
              <div><label className="text-xs font-semibold text-text-muted block mb-1">{t('common.description')}</label><input value={newDesc} onChange={e => setNewDesc(e.target.value)} className="input" /></div>
              <div className="flex gap-2 mt-1">
                <button onClick={() => setShowCreate(false)} className="btn btn-ghost flex-1 justify-center">{t('common.cancel')}</button>
                <button onClick={handleCreate} disabled={creating || !newName.trim()} className="btn btn-primary flex-1 justify-center disabled:opacity-50">{creating ? t('common.saving') : t('common.create')}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showAddIng && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowAddIng(false)}>
          <div className="bg-surface-card rounded-xl border border-border w-full max-w-lg shadow-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-border"><h2 className="text-lg font-bold text-text">{t('ingredients.add_ingredient')}</h2><span className="text-2xs text-text-ghost font-mono">{addableIngs.length}</span><button onClick={() => setShowAddIng(false)} className="text-text-ghost bg-transparent border-none cursor-pointer"><X size={18} /></button></div>
            <div className="p-3 border-b border-border">
              <div className="relative mb-2"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-ghost" /><input value={ingSearch} onChange={e => setIngSearch(e.target.value)} placeholder={t('common.search')} className="input pl-9" autoFocus /></div>
              <div className="flex gap-1 flex-wrap">{[{ k: '', l: t('common.all') }, { k: 'forage', l: '🌿 Forage' }, { k: 'concentrate', l: '🌾 Concentrate' }, { k: 'energy', l: '⚡ Energy' }, { k: 'protein', l: '🥩 Protein' }, { k: 'byproduct', l: '♻ Byproduct' }, { k: 'mineral', l: '🧪 Mineral' }].map(f => (<button key={f.k} onClick={() => setIngCatFilter(f.k)} className={`px-2.5 py-1 rounded text-2xs font-semibold transition-all border cursor-pointer ${ingCatFilter === f.k ? 'border-brand bg-brand/10 text-brand' : 'border-border text-text-ghost hover:border-border-light bg-transparent'}`}>{f.l}</button>))}</div>
            </div>
            <div className="flex-1 overflow-auto">{addableIngs.length > 0 ? addableIngs.map(ing => (<div key={ing.id} onClick={() => addIngToPremix(ing.id)} className="flex items-center gap-3 px-4 py-2 border-b border-border/5 hover:bg-[#253442] cursor-pointer"><div className="flex-1"><div className="text-sm font-semibold text-text-dim">{ing.name}</div><div className="text-2xs text-text-ghost">{ing.particle_class === 'forage' ? 'F' : 'C'} · {ing.category} · CP {ing.cp_pct || 0}% · ME {ing.me_mj || 0}</div></div>{prices[ing.id] && <span className="text-xs font-mono text-status-amber">${prices[ing.id].toFixed(0)}/t</span>}<Plus size={14} className="text-brand" /></div>)) : <div className="px-4 py-8 text-center text-sm text-text-ghost">{t('common.no_results')}</div>}</div>
          </div>
        </div>
      )}
    </div>
