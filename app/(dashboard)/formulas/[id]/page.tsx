...
import { Plus, Sparkles, Save, Lock, Unlock, X, Search, ChevronDown, ChevronUp, Loader2 } from 'lucide-react'
...
export default function FormulaBuilderPage() {
  ...
  function calcNutrient(key: string): number { return ings.reduce((s, fi) => s + (fi.ingredient?.[key] || 0) * (fi.inclusion_pct || 0) / 100, 0) }
  const costPerTonne = ings.reduce((s, fi) => s + (prices[fi.ingredient_id] || 0) * (fi.inclusion_pct || 0) / 100, 0)
  const ca = calcNutrient('ca_pct'), p = calcNutrient('p_pct'), caP = p > 0 ? ca / p : 0
  ...
  async function handleSave() {
    ...
    if (ings.length > 0) {
      await supabase.from('formula_ingredients').insert(ings.map(fi => ({
        ...
        inclusion_kg: fi.inclusion_pct / 100 * (formula?.batch_size_kg || 1000), cost_per_tonne: prices[fi.ingredient_id] || null, locked: fi.locked,
      })))
    }
    await supabase.from('formulas').update({ total_cost_per_tonne: costPerTonne, total_cp_pct: calcNutrient('cp_pct'), total_me_mj: calcNutrient('me_mj') }).eq('id', params.id)
  ...
  function buildAiContext() {
    const ingList = ings.map(fi => `- ${fi.ingredient?.name}: ${fi.inclusion_pct}% (${fi.locked?'LOCKED':'flexible'}) ${prices[fi.ingredient_id]?'$'+prices[fi.ingredient_id]+'/t':''}`).join('\n')
    const nutProfile = nutrients.map(nt => `${nt.label}: ${calcNutrient(nt.key).toFixed(2)}${nt.unit}`).join(', ')
    const reqList = requirements.map(r => `${r.nutrient}: min ${r.min??'-'} / target ${r.target} / max ${r.max??'-'} ${r.unit}${r.critical_max?' [CRITICAL >'+r.critical_max+']':''}${r.critical_min?' [CRITICAL <'+r.critical_min+']':''}`).join('\n')
...
BATCH: ${formula.batch_size_kg || 1000}kg
${prodInfo ? 'PRODUCTION LEVEL: ' + prodInfo : ''}
...
${nutProfile}
Ca:P ratio: ${caP.toFixed(2)}:1
Total inclusion: ${totalPct.toFixed(1)}%
Cost: $${costPerTonne.toFixed(0)}/tonne

TARGET REQUIREMENTS (${stageName}):
${reqList}
  ...
  async function handleAiReview() {
    ...
    try {
      const res = await fetch('/api/ai/chat', {
        ...
        body: JSON.stringify({
          prompt: `Review this formula in detail. Check nutrient balance vs requirements, identify any safety rule violations, suggest cost optimization opportunities, and flag any issues with the production level targets. Be specific with numbers and Australian context.\n\n${buildAiContext()}`,
        })
      })
      const data = await res.json()
      setAiReview(data.response || 'No response received.')
    ...
    } catch { setAiReview('Error connecting to AI. Check your API key.') }
    setAiLoading(false)
  }
  ...
  return (
    <div className="p-4 max-w-[1400px] h-[calc(100vh-32px)] flex flex-col">
        ...
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <select value={formula.status} onChange={e => updateStatus(e.target.value)}
              className={`text-2xs px-2 py-0.5 rounded font-bold font-mono uppercase border-none cursor-pointer ${formula.status==='draft'?'bg-status-amber/15 text-status-amber':formula.status==='review'?'bg-status-blue/15 text-status-blue':'bg-brand/15 text-brand'}`}>
      ...
      <div className="mb-2">
        ...
        {showProduction && (
          <div className="grid grid-cols-6 gap-2 mt-2 p-3 bg-surface-card rounded-lg border border-border">
            {prodFields.map(f => (
              <div key={f.key}>
                <label className="text-2xs text-text-ghost block mb-0.5">{f.label} <span className="text-text-ghost/50">({f.unit})</span></label>
                <input value={production[f.key]||''} onChange={e => setProduction({...production, [f.key]: e.target.value})}
                  placeholder={f.placeholder} className="w-full px-2 py-1.5 rounded border border-border bg-surface-deep text-text-dim text-sm font-mono outline-none focus:border-border-focus" />
              </div>
            ))}
          </div>
        )}
      ...
      <div className="grid grid-cols-[1fr_320px] gap-3 flex-1 min-h-0">
        ...
        <div className="card flex flex-col">
          ...
          <div className="flex-1 overflow-auto">
            {ings.map((fi,idx)=>{const ing=fi.ingredient;const price=prices[fi.ingredient_id];if(!ing)return null;return(
              <div key={fi.id||idx} className="grid grid-cols-[24px_1fr_80px_60px_50px_24px] px-3 py-2 border-b border-border/5 items-center gap-1.5">
        ...
        <div className="flex flex-col gap-0 overflow-hidden">
          ...
          {rightTab==='nutrients'&&(
            <div className="card p-3 flex-1 overflow-auto">
              <div className="text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Nutrient Profile</div>
          ...
          {rightTab==='cost'&&(
            <div className="card p-3 flex-1 overflow-auto">
              <div className="grid grid-cols-2 gap-2 mb-3">
                {[['Per Tonne','$'+costPerTonne.toFixed(0)],['Per kg','$'+(costPerTonne/1000).toFixed(3)],['Per Head/Day','$'+(costPerTonne/1000*(parseFloat(production.dmi)||22)).toFixed(2)],['Per L Milk','$'+(costPerTonne/1000*(parseFloat(production.dmi)||22)/(parseFloat(production.milk_yield)||28)).toFixed(3)]].map(([l,v])=>(
                  <div key={l} className="bg-surface-deep rounded-lg p-2"><div className="text-2xs text-text-ghost font-semibold uppercase">{l}</div><div className="text-lg font-bold text-status-amber font-mono mt-0.5">{v}</div></div>
                ))}
              </div>
              <div className="text-2xs font-bold text-text-ghost uppercase mb-2">Breakdown</div>
              {ings.filter(fi=>fi.inclusion_pct>0).sort((a,b)=>(prices[b.ingredient_id]||0)*b.inclusion_pct/100-(prices[a.ingredient_id]||0)*a.inclusion_pct/100).map(fi=>{
                const price=prices[fi.ingredient_id]||0;const ingCost=price*fi.inclusion_pct/100;const pctCost=costPerTonne>0?ingCost/costPerTonne*100:0
                return(<div key={fi.id} className="flex items-center gap-2 py-1"><span className="text-xs text-text-dim flex-1 truncate">{fi.ingredient?.name}</span><div className="w-16 h-1 bg-surface-deep rounded-sm overflow-hidden"><div className="h-full bg-status-amber rounded-sm" style={{width:`${pctCost}%`}}/></div><span className="text-xs font-mono text-status-amber w-12 text-right">${ingCost.toFixed(0)}</span><span className="text-2xs font-mono text-text-ghost w-8 text-right">{pctCost.toFixed(0)}%</span></div>)
              })}
            </div>
          )}
        </div>
      ...
      {showAi&&(<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={()=>setShowAi(false)}><div className="bg-surface-card rounded-xl border border-border w-full max-w-2xl shadow-2xl max-h-[80vh] flex flex-col" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div className="flex items-center gap-2.5"><div className="w-7 h-7 rounded-lg bg-gradient-to-br from-brand to-brand-dark flex items-center justify-center"><Sparkles size={14} className="text-white"/></div><div><div className="text-base font-bold text-text">Optia AI Review</div><div className="text-2xs text-text-ghost">{formula.name}</div></div></div>
          <button onClick={()=>setShowAi(false)} className="text-text-ghost bg-transparent border-none cursor-pointer"><X size={18}/></button>
        </div>
        <div className="flex-1 overflow-auto p-4">
          {aiLoading&&!aiReview&&<div className="flex items-center gap-3 text-text-ghost"><Loader2 size={16} className="animate-spin"/> Analyzing formula against {stageName} requirements...</div>}
          {aiReview&&<div className="text-sm text-text-dim leading-relaxed whitespace-pre-wrap" dangerouslySetInnerHTML={{__html:aiReview.replace(/\*\*(.*?)\*\*/g,'<b>$1</b>').replace(/\n/g,'<br/>')}}/>}
        </div>
        <div className="p-4 border-t border-border flex gap-2">
          <input value={aiQuestion} onChange={e=>setAiQuestion(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleAiQuestion()} placeholder="Ask a follow-up question..." className="flex-1 px-3 py-2 rounded-lg border border-border bg-surface-deep text-text-dim text-sm outline-none focus:border-border-focus"/>
          <button onClick={handleAiQuestion} disabled={aiLoading} className="btn btn-primary btn-sm">{aiLoading?<Loader2 size={14} className="animate-spin"/>:'Ask'}</button>
          <button onClick={handleAiReview} disabled={aiLoading} className="btn btn-ai btn-sm">Re-review</button>
        </div>
      </div></div>)}
    </div>
  )
