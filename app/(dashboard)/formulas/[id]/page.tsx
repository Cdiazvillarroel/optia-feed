...
export default function FormulasPage() {
  ...
  async function loadData() {
    ...
    if (!user) return
    const { data: f } = await supabase.from('formulas').select('*, client:nutrition_clients(name)').eq('nutritionist_id', user.id).not('status', 'eq', 'archived').order('updated_at', { ascending: false })
    setFormulas(f || [])
  ...
  async function handleCreate() {
    ...
    const { data } = await supabase.from('formulas').insert({
      ...
      species: newSpecies, production_stage: newStage, batch_size_kg: parseInt(newBatch) || 1000,
      status: 'draft', version: 1,
    }).select().single()
    setCreating(false)
    if (data) { setShowCreate(false); router.push(`/formulas/${data.id}`) }
  }
  ...
  return (
    <div className="p-7 max-w-[1200px]">
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={() => { setNewName(''); setNewClientId(''); setNewSpecies('cattle'); setNewStage(''); setNewBatch('1000'); setNewAnimalGroupId(''); setShowCreate(true) }} className="btn btn-primary"><Plus size={14} /> New Formula</button>
      </div>
      ...
      <div className="flex gap-2.5 mb-4">
        ...
        <div className="flex gap-1">{['', 'draft', 'review', 'approved', 'active'].map(s => (<button key={s} onClick={() => setFilterStatus(s)} className={`filter-pill ${filterStatus === s ? 'active' : ''}`}>{s || 'All'}</button>))}</div>
      </div>
      ...
      <div className="card">
        ...
        {filtered.map(f => (
          <Link key={f.id} href={`/formulas/${f.id}`} className="grid grid-cols-[2fr_1fr_1.5fr_1fr_50px] px-4 py-3 border-b border-border/5 gap-2 items-center hover:bg-[#253442] transition-colors no-underline">
            ...
            <span className={`text-2xs px-2 py-0.5 rounded font-bold font-mono uppercase w-fit ${f.status === 'draft' ? 'bg-status-amber/15 text-status-amber' : f.status === 'review' ? 'bg-status-blue/15 text-status-blue' : 'bg-brand/15 text-brand'}`}>{f.status}</span>
            <span className="text-text-ghost">&rsaquo;</span>
          </Link>
        ))}
        {filtered.length === 0 && <div className="px-4 py-12 text-center text-sm text-text-ghost">{search || filterStatus ? 'No formulas match.' : 'No formulas yet.'}</div>}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-surface-card rounded-xl border border-border w-full max-w-lg p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div><label className="text-xs font-semibold text-text-muted block mb-1">Client</label><select value={newClientId} onChange={e => setNewClientId(e.target.value)} className="input"><option value="">No client (template)</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>

              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-semibold text-text-muted block mb-1">Species *</label><select value={newSpecies} onChange={e => setNewSpecies(e.target.value)} className="input">{SPECIES_OPTIONS.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select></div>
                <div>
              ...
              <div className="flex gap-2 mt-1">
                ...
                <button onClick={handleCreate} disabled={creating || !newName.trim() || !newStage} className="btn btn-primary flex-1 justify-center disabled:opacity-50">{creating ? 'Creating...' : 'Create Formula'}</button>
              </div>
            </div>
          </div>
        </div>
