...
export function ClientsList({ clients, animalCounts, formulaCounts }: Props) {
  ...
  return (
    <div className="p-7 max-w-[1200px]">
      ...
      <div className="flex gap-2.5 mb-4 items-center">
        ...
        <div className="flex gap-1">
          <button
            ...
            className={`filter-pill ${!speciesFilter ? 'active' : ''}`}
          >All</button>
          {SPECIES_OPTIONS.map((sp) => (
            <button
              key={sp.key}
              onClick={() => setSpeciesFilter(speciesFilter === sp.key ? null : sp.key)}
              className={`filter-pill ${speciesFilter === sp.key ? 'active' : ''}`}
            >{sp.key}</button>
          ))}
        </div>
      </div>
      ...
      <div className="card">
        ...
        {filtered.length > 0 ? filtered.map((c) => (
          <Link
            key={c.id}
            href={`/clients/${c.id}`}
            className="grid grid-cols-[2fr_1fr_1fr_80px_80px_40px] px-4 py-3 border-b border-border/5 gap-2 items-center hover:bg-[#253442] transition-colors no-underline"
          >
            ...
            <div className="flex gap-1">
              {(c.species as string[]).map((s) => {
                ...
                return sp ? (
                  <span key={s} className={`inline-flex items-center justify-center w-[22px] h-[22px] rounded text-xs font-bold font-mono ${sp.color}`}>
                    {sp.label}
                  </span>
                ) : null
              })}
            ...
            <span className="text-text-ghost text-right">›</span>
          </Link>
        )) : (
      ...
      <div className="mt-3 text-xs text-text-ghost">
        {filtered.length} client{filtered.length !== 1 ? 's' : ''}{search || speciesFilter ? ' (filtered)' : ''}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-surface-card rounded-xl border border-border w-full max-w-lg p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowForm(false)} className="text-text-ghost hover:text-text-muted bg-transparent border-none cursor-pointer">
                <X size={18} />
              </button>
            </div>
            ...
            <form action={handleSubmit} className="flex flex-col gap-3.5">
              <div>
                ...
                <input name="name" required className="input" placeholder="e.g. Kapram Dairy" />
              </div>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded border text-sm font-semibold capitalize transition-all cursor-pointer
                        ${selectedSpecies.includes(sp)
                          ...
                          : 'border-border bg-surface-bg text-text-faint hover:border-border-light'}`}
                    >
                      {sp === 'cattle' ? '🐄' : sp === 'pig' ? '🐷' : sp === 'poultry' ? '🐔' : '🐑'} {sp}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-text-muted block mb-1">Contact Name</label>
                  <input name="contact_name" className="input" placeholder="James Mitchell" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-text-muted block mb-1">Contact Phone</label>
                  <input name="contact_phone" className="input" placeholder="+61 3 9876 5432" />
                </div>
              </div>

              <div>
                <label className="text-xs font-semibold text-text-muted block mb-1">Contact Email</label>
                <input name="contact_email" type="email" className="input" placeholder="james@farm.com.au" />
              </div>

              <div>
                <label className="text-xs font-semibold text-text-muted block mb-1">Location</label>
                <input name="location" className="input" placeholder="Gippsland, VIC" />
              </div>

              <div>
                <label className="text-xs font-semibold text-text-muted block mb-1">Notes</label>
                <textarea name="notes" className="input min-h-[60px] resize-y" placeholder="Any notes about this client..." />
              </div>

              <div className="flex gap-2 mt-2">
                <button type="button" onClick={() => setShowForm(false)} className="btn btn-ghost flex-1 justify-center">
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={formLoading || selectedSpecies.length === 0}
