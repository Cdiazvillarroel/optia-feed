...
export default function ReportsPage() {
  ...
  function generateFormulaReport() {
    ...
    const nutRows = nutData.map(([l,v,u])=>`<tr><td class="bold">${l}</td><td class="right mono bold">${typeof v==='number'?((l as string).includes('Ca:P')?v.toFixed(2):v<1?v.toFixed(3):v.toFixed(1)):v}</td><td class="center gray">${u}</td></tr>`).join('')
    ...
    <div class="two-col"><div><table><thead><tr><th>Nutrient</th><th class="right">Value</th><th class="center">Unit</th></tr></thead><tbody>${nutRows.slice(0, Math.ceil(nutData.length/2))}</tbody></table></div><div><table><thead><tr><th>Nutrient</th><th class="right">Value</th><th class="center">Unit</th></tr></thead><tbody>${nutRows.slice(Math.ceil(nutData.length/2))}</tbody></table></div></div>
