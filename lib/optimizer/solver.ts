// =====================================================
// Optia LP Solver — Linear Programming with heuristic fallback
// =====================================================
// Variables: x_i = fraction of ingredient i (0..1, sum to 1)
// Objective: minimize Σ (x_i × price_DM_i)
// Constraints:
//   - Σ x_i = 1
//   - Σ (x_i × nutrient_i) ≥ requirement_min  (and ≤ max)
//   - min_pct/100 ≤ x_i ≤ max_pct/100
//   - locked ingredients: x_i = current_pct/100
// =====================================================
...
export function solveLeastCost(input: OptimizerInput): OptimizerResult {
  ...
  // LP infeasible → try heuristic as last resort
...
function tryLP(input: OptimizerInput): OptimizerResult {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // DM-basis price: cost per tonne of DM
    ...
    // Per-ingredient bounds (encoded as separate constraints)
    if (ing.locked) {
  ...
  const solution = ingredients.map((_, i) => {
    ...
    return Math.round(frac * 1000) / 10 // %DM, 1 decimal
  })
...
// ============================================================
// HEURISTIC FALLBACK (preserves legacy behavior when LP infeasible)
// ============================================================
function tryHeuristic(
  ...
  // Pairwise binary-search exchanges
